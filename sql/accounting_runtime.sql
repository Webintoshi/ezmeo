-- Accounting runtime tables for Muhasebe + Fatura Entegrasyonu
-- Run in Supabase SQL editor before enabling accounting features.
...
on public.accounting_provider_connections
for all
...
on public.accounting_invoice_queue
for all
using (auth.role() = 'service_role')
with check (auth.role() = 'service_role');
...
on public.accounting_sync_jobs
for all
using (auth.role() = 'service_role')
with check (auth.role() = 'service_role');
...
on public.accounting_entity_mappings
for all
using (auth.role() = 'service_role')
with check (auth.role() = 'service_role');
