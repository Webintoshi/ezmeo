...
export function Hero() {
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (data?.value && (data.value as HeroSettings).slides?.length > 0) {
          const dbSlides = (data.value as HeroSettings).slides.filter(
            (s: HeroSlide) => s.desktop && s.mobile
          );
          setSlides(dbSlides);
        }
      } catch (error) {
  ...
  if (loading) {
    return (
      <section className="relative h-[500px] sm:h-[600px] md:h-[700px] lg:h-[850px] overflow-hidden bg-gray-900">
    ...
    return (
      <section className="relative h-[500px] sm:h-[600px] md:h-[700px] lg:h-[850px] overflow-hidden bg-gray-900">
        <div className="absolute inset-0 bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center">
          <p className="text-white/50">Hero banner görselleri eklemek için admin panelini kullanın</p>
        </div>
      </section>
    );
  }
