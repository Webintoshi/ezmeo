...
// 3. Define Product Data (Copied from lib/products.ts to ensure standalone execution)
...
async function seed() {
    try {
        console.log("Starting DB seed from script...");
        ...
        console.log(`Inserting ${DEFAULT_PRODUCTS.length} products...`);
        for (const product of DEFAULT_PRODUCTS) {
            const { variants, ...productData } = product;

            // Filter payload to match valid DB columns ONLY
            const dbProduct = {
                id: product.id,
                name: product.name,
                slug: product.slug,
                ...
                // seo_title and seo_description omitted if undefined
                // Removed non-existent columns: subcategory, nutritional_info, flags, ratings, etc.
            };
            ...
            if (insertProductError) {
                console.error(`Failed to insert product ${product.name}:`, insertProductError);
                continue;
            }
            ...
            if (variants && variants.length > 0) {
                const dbVariants = variants.map((v) => ({
                    ...
                    name: v.name,
                    // sku: text (optional in schema)
                    sku: v.sku,
                    price: v.price,
                    original_price: v.originalPrice,
                    stock: v.stock,
                    weight: v.weight ? String(v.weight) : null // schema has weight TEXT
                }));
                ...
                if (insertVariantError) {
                    console.error(`Failed to insert variants for ${product.name}:`, insertVariantError);
                }
            }
        }
