...
export default function CmsDashboard() {
    ...
    const stats = [
        { label: "Blog Yazıları", count: blogPosts.length, icon: PenTool, color: "text-blue-600", bg: "bg-blue-50" },
        { label: "Statik Sayfalar", count: cmsPages.length, icon: FileText, color: "text-purple-600", bg: "bg-purple-50" },
        { label: "Taslaklar", count: blogPosts.filter(p => p.status === "draft").length + cmsPages.filter(p => p.status === "draft").length, icon: Clock, color: "text-amber-600", bg: "bg-amber-50" },
        { label: "Yayında", count: blogPosts.filter(p => p.status === "published").length + cmsPages.filter(p => p.status === "published").length, icon: Eye, color: "text-emerald-600", bg: "bg-emerald-50" },
    ];
    ...
    return (
        <div className="min-h-screen bg-gray-50/50 p-6 md:p-8 space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 tracking-tight">İçerik Yönetimi (CMS)</h1>
                    <p className="text-sm text-gray-500 mt-1">Blog yazılarını ve kurumsal sayfalarınızı buradan yönetin.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
                    ...
                    <div className="p-8">
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500 mt-0.5">Haberler, duyurular ve bilgilendirici içerikler.</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3 mb-8">
                            ...
                            <div className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100">
                                <span className="text-sm font-medium text-gray-600">Aktif Kategori</span>
                ...
                <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden group hover:shadow-md transition-all">
                    <div className="p-8">
                        <div className="flex items-center gap-4 mb-6">
                            <div className="w-14 h-14 bg-purple-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-purple-200 group-hover:scale-110 transition-transform">
                                <FileText className="w-6 h-6" />
                            </div>
                            <div>
                                <h2 className="text-xl font-bold text-gray-900">Sayfa Yönetimi</h2>
                                <p className="text-sm text-gray-500 mt-0.5">Hakkımızda, İletişim, Sözleşmeler ve daha fazlası.</p>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-3">
                            ...
                            <Link
                                ...
                                className="px-4 py-3 bg-purple-50 text-purple-600 rounded-xl hover:bg-purple-100 transition-all"
                            >
                                <Plus className="w-5 h-5" />
                            </Link>
                        </div>
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="max-w-xl">
                        ...
                        <p className="text-gray-300 text-sm leading-relaxed">
                            SEO Killer modülü ile entegre çalışarak içeriklerinizdeki anahtar kelimeleri otomatik optimize edin. Blog yazılarınızda ürünlerinize link vererek satışlarınızı artırabilirsiniz.
                        </p>
                    </div>
                    <Link
